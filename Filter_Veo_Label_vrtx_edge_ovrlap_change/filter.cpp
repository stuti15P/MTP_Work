...
void printingAndWritingFinalStatistics(int choice,unsigned long looseCount,unsigned long strictCount, unsigned long vrtxOvrlapEdgeStrict, unsigned long staticCount,bool isBucket,unsigned long partitionCount,unsigned long dynamicCount,bool mismatch,unsigned long mismatchCount,unsigned long simPairCount,int totalTimeTaken,const string res_dir,vector<long long int>& global_score_freq,unordered_map<unsigned, vector<pair<unsigned, double>>>& g_res)
{
	...
	if(choice ==5)
	{
		cout<<" Vertex Overlap Edge Strict Count : "<<vrtxOvrlapEdgeStrict<<endl;
	}
	if(mismatch)
...
int main(int argc, char const *argv[])
{
	...
	for(int g1 = 0; g1 < graph_dataset.size(); g1++)
	{
		...
		for(int g2 = g1-1; g2 >= 0; g2--)
		{
			...
			if(choice > 1 && choice != 5) // Strict Filter
			{
			...
			if(choice ==5) // vertex overlap + strict for Edge only (Min  Edges count)
			{
				//cout<<"g1 = "<<g1<<" g2 "<<g2<<"calling intersection_vertices \n";
				double common_vrtx = intersection_vertices(graph_dataset[g1], graph_dataset[g2]);  //This is Exact Vertex Overlap.
				double max_edge_intersection = min(graph_dataset[g1].edgeCount, graph_dataset[g2].edgeCount); // This is Max Possible  Edge overlap.
				double vrtx_ovrlap_edge_strict = (double)200.0 *((common_vrtx + max_edge_intersection)/(currSize+PrevSize));
    ...
    printingAndWritingFinalStatistics(choice,looseCount,strictCount, vrtxOvrlapEdgeStrict, staticCount,isBucket,partitionCount,dynamicCount,mismatch,mismatchCount,simPairCount,totalTimeTaken,res_dir,global_score_freq,g_res);
