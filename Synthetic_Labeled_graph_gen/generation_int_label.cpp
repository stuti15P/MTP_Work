...
void createGraph(unsigned gnumber,unsigned &nv,unsigned &ne,unsigned long maxV,unsigned long minV,unsigned long maxE,unsigned long minE,vector<unsigned>& vertex_ids,unordered_map<unsigned,unordered_set<unsigned> >& edges,unsigned long labelCount,unordered_map<unsigned, unsigned> &vrtxtocharmap, map<pair<unsigned, unsigned>,int>& edgetocharmap);
...
void writeToFile(unsigned gnumber,unsigned nv,ofstream &datafile,vector<unsigned>& vertex_ids,unordered_map<unsigned,unordered_set<unsigned> >& edges,int NGraphs,unordered_map<unsigned, unsigned> &vrtxtocharmap, map<pair<unsigned, unsigned>,int>& edgetocharmap);
...
int main(int argc, char const *argv[])
{
    ...
    for(int g = 0; g < NGraphs; g++)
   {
		 ...
		 unordered_map<unsigned, unsigned> vrtxtocharmap;
...
unsigned getRandomVertexIntLabel(unsigned long maxV){
        return rand()%(maxV)+1;
}
...
void getVertex_ids(unsigned nv,vector<unsigned> &vertex_ids,unsigned long maxV,unsigned long labelCount,unordered_map<unsigned, unsigned> &vrtxtocharmap){

	std::random_device rd;
	std::mt19937 gen(rd());
//    std::discrete_distribution<> d({2, 3, 20, 0,0, 0, 0 ,15, 0, 0, 0, 0, 0, 20, 15, 5 , 0 , 0, 10, 5, 5 });  // 4 labels with 4 types of probability
    	unsigned vlbl_count =45;
